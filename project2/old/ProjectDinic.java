...
class Vertex {
	...
	private Vertex(int key, int level) {
		_key   = key;
		_level = level;
	}
...
class Graph {
	...
	private List<ArrayList<Edge>> _edges;

	private Graph(int size, List<Vertex> vertices, List<ArrayList<Edge>> edges) {
		_vertexNo = size;
		_vertices = vertices;
		_edges    = edges;
	...
	public Graph(int size) {
		this(size, new ArrayList<Vertex>(), new ArrayList<ArrayList<Edge>>());
	}


	public List<Vertex> vertices() { return _vertices; }

	public List<Edge> adjacency(Vertex v) { return _edges.get(v.key()); }
	...
	public Vertex source() { return _vertices.get(0); }
	public Vertex target() { return _vertices.get(_vertexNo - 1); }
	...
	public void addVertex(Vertex v) {
		if (v.isValid())
			_vertices.add(v.key(), v);
		else
			System.out.println("Invalid vertex");
	}

	public void addEdge(Edge e) {
		_edges.get(e.source().key()).add(e);
	}
...
class MinCutMaxFlow {
	...
	public void addStartingFlow() {
		...
		for (Edge sv : _g.adjacency(s)) {
			for (Edge cand : _g.adjacency(sv.target())) {
				if (cand.target().equals(t)) {
					vt = cand;
					break;
				}
			}
	...
	public int dinicDFS(Vertex v, int flow, int[] restart) {
		...
		int size = _g.adjacency(v).size();

		for ( ; restart[v.key()] < size ; ++restart[v.key()]) {
			Edge e = _g.adjacency(v).get(restart[v.key()]);
