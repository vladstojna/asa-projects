...
class Graph {
	...
	public:
		...
		Graph(int nv, int ne) :
			...
			_edgeNo(ne),
			_sccNo(0),
			_vertices(new Vertex*[nv]),
			_connects(new Vertex*[nv]),
			_output(ne) {}
		...
		Vertex **vertices()      const { return _vertices;          }
		...
		void getOutput() {
			int i = 0;
			for (int j = 0; j < _vertexNo; j++)
				for (Vertex *w : _vertices[j]->adjacency()) {
					if (_vertices[j]->minKey() != w->minKey()) {
						_output[i] = new Edge(_vertices[j]->minKey(), w->minKey());
						i++;
					}
				}
			
			std::set<Edge*, comparator> s;
			unsigned size = _edgeNo;
...
class SCCFinder {
	...
	public:
		...
		void tarjan() {
			for (int j = 0; j < _graph->vertexNo(); j++)
				if (_graph->vertex(j + 1)->undiscovered())
					strongconnect(_graph->vertex(j + 1));
		}
